    ...
    class Program
    {
        ...
        public static void squareFill(int startX, int endX, int startY, int endY, double power)
        {
            ...
            for (float x = startX; x < endX + 1; x++)
            {
                ...
                {
                    z = new Quaternion(0,0,0,0);
                    float newX = Convert.ToSingle((x - Global.relativeXAxis) / (Global.xSize / 8.6));
                    float newY = Convert.ToSingle((y - Global.relativeYAxis) / (Global.ySize / 4.3));

                    c = new Quaternion(Convert.ToSingle(0.3),Convert.ToSingle(0.3),newX, newY);
                    ...
                    {
                        z = Quaternion.Add(Quaternion.Multiply(z, z), c);
        ...
        private static void makeSetOfPower(double power)
        {
            ...
            Global.mandelbrotImage.Save(power + ".bmp");
        }
    }
}
